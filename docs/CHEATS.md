...
`Show cheats` reaches the core and nothing reads it. `cheat_osd`, `cheat_font`
and `cheat_titles` are not ported, and §6 of `PLAN.md` has an unresolved problem
there: the overlay is parameterised for a fixed 160x144 Game Boy raster and the
PC Engine has no fixed resolution. Until that lands the toggle does nothing.
...
| Menu entries | `interact.json`, ids 81 and 82 |
